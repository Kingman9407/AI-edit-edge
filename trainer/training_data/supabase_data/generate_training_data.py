...
Usage:
    ...
    python generate_training_data.py --batch 20   # generate 20 examples
"""
...
def main(batch_size: int = DEFAULT_BATCH_SIZE) -> int:
    ...
    """
    print("=" * 60)
    print("    HORNET AI — SYNTHETIC DATA GENERATOR (NVIDIA GLM) FOR TRAINING")
    print("=" * 60)
    ...
    print(f"✅ Generated {len(examples)} examples. Pushing directly to Supabase as passing rows...")
    ...
    for ex in examples:
        ...
        try:
            supabase.table("ai_logs").insert({
                ...
                "score":           100.0,
                "is_correct":      True,
                "model_name":      f"synthetic-{NVIDIA_MODEL}-training",
            }).execute()
            success_count += 1
        except Exception as e:
    ...
    print(f"\n{'='*60}")
    print(f"🎉 Requested {batch_size} examples. Inserted {success_count} synthetic training logs into Supabase 'ai_logs'!")
    print(f"{'='*60}\n")
    return success_count


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate synthetic training data via NVIDIA GLM API and insert to Supabase")
